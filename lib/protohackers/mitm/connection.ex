defmodule Protohackers.MITM.Connection do
  ...
  # active incoming_sockets + this is a controlling process
  # -> incoming_socket events sent here via erlang messages
  @impl true
  ...
  def handle_info(
    ...
    # small optimisation, put back-pressure feedback before processing data
    # since by the time data is processed, next message has time to construct itself
    Logger.debug("Incoming data #{inspect(data)}")
  ...
  def handle_info(
    ...
    Logger.debug("Outgoing data #{inspect(data)}")
    data = Protohackers.MITM.Boguscoin.rewrite_addresses(data)
